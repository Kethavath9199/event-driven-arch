...
describe('EnumFilterComponent', () => {
  ...
  describe('getModel', () => {
    it('returns a single condition if filtertype is single', () => {
      ...
      const filterType1: keyof SearchTypeEnum = 'equals';
      const filter1 = 'search';
      const filterModel: FilterModelType = {
        type: conditional,
        condition1: { [filterType1 as string]: filter1 },
    ...
    it('returns a both conditions if filtertype is AND', () => {
      ...
      const filterType2: keyof SearchTypeEnum = 'equals';
      const filter2 = 'search2';
      const filterModel: FilterModelType = {
        type: conditional,
        condition1: { [filterType1 as string]: filter1 },
        condition2: { [filterType2 as string]: filter2 },
        isDate: false,
      };
      jest.spyOn(component, 'isFilterActive').mockReturnValue(true);
      component.conditional = conditional;
  ...
  describe('setModel', () => {
    it('sets model to empty string if null is supplied', () => {
      ...
      expect(component.filter1).toBe(null);
      expect(component.filter2).toBe(null);
    });
  });
