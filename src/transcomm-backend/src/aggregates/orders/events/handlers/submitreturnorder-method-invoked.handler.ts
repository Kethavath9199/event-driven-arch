...
@EventsHandler(SubmitReturnOrderMethodInvokedEvent)
export class SubmitReturnOrderMethodInvokedHandler implements IEventHandler<SubmitReturnOrderMethodInvokedEvent>{
    ...
    async handle(event: SubmitReturnOrderMethodInvokedEvent): Promise<void> {
      ...
      //  );
        ...
        if (event.retriedBy) {
            ...
            const updateManualRetryViewRequest = closeManualRetryView(event.orderNumber, event.ecomCode, invoiceId, ContractMethod.SubmitOrderModeReturn, event.remark)
